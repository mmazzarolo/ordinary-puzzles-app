...
export const Tutorial: FC = observer(function () {
  ...
  const [descriptionLayout, setDescriptionLayout] = useState<LayoutRectangle | null>(null);
  ...
  const availableVerticalSpace = descriptionLayout
    ? screenHeight - descriptionLayout.height - getBottomNavHeight(scale) - metrics.screenMargin * 8 // Additional vertical padding
    : undefined;
  ...
  const handleBoardClearedAnimStart = () => {
    ...
    fadeInterfaceAnim.setup({ duration: fadeInterfaceOutAnimDuration, toValue: 0 }).start();
  };
